...
import { ResultsService } from '../../../backend/src/results/results.service';
...
describe('ResultsService', () => {
      ...
      providers: [
        ResultsService,
        {
          ...
          useValue: {
            ...
            result: { findMany: jest.fn(), findFirst: jest.fn(), create: jest.fn(), update: jest.fn() },
          },
        },
        {
  ...
  describe('findAll', () => {
    it('should return all results for SUPER_ADMIN', async () => {
      const mockResults = [{ id: 'result-1', status: 'DRAFT' }];
      prisma.result.findMany.mockResolvedValue(mockResults as any);
  ...
  describe('findOne', () => {
    it('should throw NotFoundException if result not found', async () => {
      ...
      await expect(service.findOne('invalid-id', mockSuperAdminRequester)).rejects.toThrow('Result not found');
    });
  });
});
